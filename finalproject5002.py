...
#imports the spotify data file, change to the name of your file or change your file name to this
cols = ["Genre","danceability"]
#specifies which columns in the data file we care about for the data frame (table)
...
def evidence(genre,df):
    ...
    df_songs_no_dance = df[df.danceability < 0.7]
    c = sum(df_songs_no_dance.Genre == genre)   # number of songs that are genre (ex. 'rock') and not dancible
    x = sum(df['highdance'] ==False)            # number of not danceable songs
